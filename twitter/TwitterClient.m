...
- (void)newTweetWithText:(NSString *)tweetText success:(void (^)(AFHTTPRequestOperation *operation, id response))success failure:(void (^)(AFHTTPRequestOperation *operation, NSError *error))failure {
...
- (void)replyWithText:(NSString *)tweetText forTweetId:(NSString *)tweetId success:(void (^)(AFHTTPRequestOperation *operation, id response))success failure:(void (^)(AFHTTPRequestOperation *operation, NSError *error))failure {
    NSMutableDictionary *params = [NSMutableDictionary dictionaryWithDictionary:@{@"status": tweetText}];
    if (tweetId) {
...
- (void)reTweetWithId:(NSString *)tweetId success:(void (^)(AFHTTPRequestOperation *operation, id response))success failure:(void (^)(AFHTTPRequestOperation *operation, NSError *error))failure {
    ...
    //    [self getPath:@"1.1/statuses/home_timeline.json" parameters:params success:success failure:failure];
    NSString *path1 = [@"1.1/statuses/retweet/" stringByAppendingString:tweetId];
    NSString *path = [path1 stringByAppendingString:@".json"];
    [self postPath:path parameters:params success:success failure:failure];
}

- (void)setFavoriteForTweet:(NSString *)tweetId success:(void (^)(AFHTTPRequestOperation *operation, id response))success failure:(void (^)(AFHTTPRequestOperation *operation, NSError *error))failure {
    NSMutableDictionary *params = [NSMutableDictionary dictionaryWithDictionary:@{@"id": tweetId}];
    
    //    [self getPath:@"1.1/statuses/home_timeline.json" parameters:params success:success failure:failure];
    NSString *path = @"1.1/favorites/create.json";
    [self postPath:path parameters:params success:success failure:failure];
}
